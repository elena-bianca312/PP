...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((segment (get-line-segment)) (x (get-start-point segment)) (y (get-end-point segment)))
    (sqrt (+ (sqr (- (car x) (car y))) (sqr (- (cdr x) (cdr y)))))))
...
(define (compute-f-with-step f a b step)
  (let name ((f f) (a a) (b b) (result '()))
    (if (< b 0)
        result
        (name f a (- b step) (cons (f b) result)))))
...
(define (compute-perimeter points)
  (let ((first (car points)))
    (let name ((x points) (sum 0))
      (if (null? (cdr x))
          (+ sum (distance (car x) first))
          (name (cdr x) (+ sum (distance (car x) (cadr x))))))))
...
(define (3-sequence-max numbers separator)
  (let func ((numbers numbers) (separator separator) (max 0))
    (if (null? numbers)
        max
        (let-values ([(x y) (splitf-at numbers (λ (x) (not (= x separator))))])
          (if (> (sum x) max)  
              (func (if (null? y) y (cdr y)) separator (sum x))
              (func (if (null? y) y (cdr y)) separator max))))))
