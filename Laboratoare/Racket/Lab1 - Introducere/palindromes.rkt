...
(define (deep-rev L)
  (cond
    [(null? L) '()] ; daca este lista vida
    [(list? (car L)) (append (deep-rev (cdr L)) (list (deep-rev (car L))))]
    ; daca primul element este o lista, aceasta poate contine mai multe liste si
    ; trebuie sa facem deep-rev pe ea + concatenare cu deep-rev aplicat pe restul listei
    [else (append (deep-rev (cdr L)) (list (car L)))])
    ; daca primul element este doar un element, pe acesta doar il concatenam la restul
    ; listei asupra careia aplicam metoda deep-rev
)
