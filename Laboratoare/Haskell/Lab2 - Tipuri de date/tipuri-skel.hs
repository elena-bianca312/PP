...
insertElem (BSTNode val l r) elem
  | elem == val = BSTNode val l r
  | elem < val = BSTNode val (insertElem l elem) r
  | elem > val = BSTNode val l (insertElem r elem)

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil elem = Nothing
findElem (BSTNode val l r) elem 
  | elem == val = Just elem
  | elem < val = findElem l elem
  | elem > val = findElem r elem

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BSTNode value l r) = (inorder l) ++ [value] ++ (inorder r)

mapTree :: (a -> b) -> BST a -> BST b
mapTree _ BSTNil = BSTNil
mapTree f (BSTNode value l r) = BSTNode (f value) (mapTree f l) (mapTree f r)

foldlTree :: (b -> a -> b) -> b -> BST a -> b
foldlTree _ acc BSTNil = acc
foldlTree f acc (BSTNode value l r) = foldlTree f newAcc r
  where newAcc = foldlTree f (f acc value) l
...
completeBinaryTree start = Node start (completeBinaryTree (f start)) (completeBinaryTree (g start))
