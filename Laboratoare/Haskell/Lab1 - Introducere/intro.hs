...
-- foldl
reverseList5 :: [a] -> [a]
reverseList5 l = foldl (\acc item -> [item] ++ acc) [] l

-- foldr
reverseList6 :: [a] -> [a]
reverseList6 l = foldr (\item acc -> acc ++ [item]) [] l
...
removeDuplicatesLeft l = foldl (\acc item -> 
    case elem item acc of
        True -> acc
        _    -> acc ++ [item]) [] l 
...
removeDuplicatesRight l = foldr (\item acc -> 
    case elem item acc of
        True -> acc
        _    -> [item] ++ acc) [] l 
...
findStringsLongerThanTenChars l = [x | x <- l, length x >= 10]

findStringsLongerThanTenChars2 :: [String] -> [String]
findStringsLongerThanTenChars2 l = filter (\x -> length x >= 10) l
