...
permutari4([X1, X2, X3, X4], L):- length(L, 4), member(X1, L), member(X2, L), member(X3, L), member(X4, L).
...
alternare(_, _, 0, []).
alternare(X, Y, Len, [X | Res]) :- L is Len - 1, Len > 0, alternare(Y, X, L, Res).
...
sublista(_,_) :- false.
sublista([], []).
sublista([X|List] , [X|SubList]) :- sublista(List, SubList).
...
extract([], _, _ , []).
extract(_, _, 0, []).
extract([X|List], 0, Stop, [X|Range]) :- Stop1 is Stop - 1, extract(List, 0, Stop1, Range).
extract([_|List], Start, Stop, Range) :- Start1 is Start - 1, Stop1 is Stop - 1, extract(List, Start1, Stop1, Range).
...
factorial2(0, 1).
% factorial2(N, Fact) :- N1 is N - 1, Fact1 is Fact / N, factorial2(N1, Fact1).
