...
%% myConcat(_,_,_):- fail.
myConcat([], L, L).
myConcat([H1|T1], List2, [H1|T]):- myConcat(T1, List2, T).
...
myReverseAcc([], L, L).
myReverseAcc([H|T], Acc, Rev):- myConcat([H], Acc, NewAcc), myReverseAcc(T, NewAcc, Rev).
...
%% factorial(_, _):- fail.
factorial(0, 1).
factorial(N, Fact):- N > 0, N1 is N - 1, factorial(N1, Fact1), Fact is Fact1 * N.
...
setIntersection([], _, []).
setIntersection([H1|T1], L2, [H1|I3]):- member(H1, L2), setIntersection(T1, L2, I3).
setIntersection([H1|T1], L2, I3):- \+member(H1, L2), setIntersection(T1, L2, I3).
...
%% setDiff(_, _, _):- fail.
setDiff([], _, []).
setDiff([H1|T1], L2, [H1|T3]):- \+member(H1, L2), setDiff(T1, L2, T3).
setDiff([H1|T1], L2, T3):- member(H1, L2), setDiff(T1, L2, T3).
...
%% setUnion(_, _, _):- fail.
setUnion(L1, L2, L3):- setIntersection(L1, L2, I), setDiff(L2, I, D), myConcat(L1, D, L3).
...
sameTree(Nod1, Nod2):- descendantOf(Nod2, Nod1).
sameTree(Nod1, Nod2):- isRoot(X), descendantOf(Nod1, X), descendantOf(Nod2, X).
...
%% cost(_, _, _):- fail.
cost(X, X, 0).
cost(X, Desc, 1):- arc(X, Desc).
cost(X, Asc, -1):- arc(Asc, X).
cost(X, Y, N1):- arc(X, Z), descendantOf(Y, Z), cost(Z, Y, N), N1 is N + 1.
cost(X, Y, N1):- arc(Z, X), cost(Z, Y, N), N1 is N - 1.
