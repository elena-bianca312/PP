...
%% au_pusca(-ListaNume)
%au_pusca(_) :- false.
au_pusca(ListaNume) :- findall(X, suspect(X:_:pusca), ListaNume).
...
%au_arma(_, _) :- false.
au_arma(Arma, ListaNume) :- findall(X, suspect(X:_:Arma), ListaNume).

%% au_marca/2
%% au_marca(+Arma, -ListaNume)
%au_marca(_, _) :- false.
au_marca(Marca, ListaNume) :- findall(X, suspect(X:Marca:_), ListaNume).
...
%% arme_bmw(-ListaArme)
%arme_bmw(_) :- false.
arme_bmw(ListaArme) :- findall(X, suspect(_:bmw:X), ListaArme).
...
%arme_marca(Marca, ListaArme) :- findall(X, suspect(_:Marca:X), ListaArme1), sort(ListaArme1, ListaArme).
...
marci_arma_unica(ListaMarci) :- findall(X, (suspect(_:X:_), arme_marca(X, List), length(List, Len), Len == 1), ListaMarci).
...
suspect1(Nume:Marca:Arma) :- suspect(Nume:Marca:Arma), marci_arma_unica(ListaMarci), \+member(Marca, ListaMarci).
