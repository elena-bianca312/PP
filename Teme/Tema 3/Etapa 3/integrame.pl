...
solutii_posibile_helper(integ(_, _, _, []), _, []).

solutii_posibile_helper(integ(H, W, Lista, [Word|Vocabular]), Intrebare, Final) :-
    lungime_spatiu(integ(H, W, Lista, Vocabular), Intrebare, Lungime),
    atom_chars(Word, Letters),
    \+length(Letters, Lungime),
    solutii_posibile_helper(integ(H, W, Lista, Vocabular), Intrebare, Final).

solutii_posibile_helper(integ(H, W, Lista, [Word|Vocabular]), Intrebare, [Letters|Final]) :-
    lungime_spatiu(integ(H, W, Lista, Vocabular), Intrebare, Lungime),
    atom_chars(Word, Letters),
    length(Letters, Lungime),
    solutii_posibile_helper(integ(H, W, Lista, Vocabular), Intrebare, Final).

% Pentru fiecare membru din intrebari -> Pentru fiecare intrebare aflam lista de candidati
% folosindu-ne de solutii_posibile_helper 

solutii_posibile_recursive(integ(_, _, _, _), [], []).

solutii_posibile_recursive(integ(H, W, Lista, Vocabular), [((_, _), Intrebare, _, _)|Rest], [(Intrebare, Final)|Sol]) :-
    solutii_posibile_helper(integ(H, W, Lista, Vocabular), Intrebare, Final),
    solutii_posibile_recursive(integ(H, W, Lista, Vocabular), Rest, Sol).

solutii_posibile(integ(H, W, Lista, Vocabular), Sol) :- 
    intrebari(integ(H, W, Lista, Vocabular), [((_, _), Intrebare, _, _)|Rest]), 
    solutii_posibile_recursive(integ(H, W, Lista, Vocabular), [((_, _), Intrebare, _, _)|Rest], Sol).
...
%rezolvare_helper(integ(H, W, Lista, Vocabular), [(Intrebare1, List1)|(Intrebare2, List2)|R]) :-
%    member((Intrebare, Words), Candidates),


%rezolvare(integ(H, W, Lista, Vocabular), Sol) :-
%    intrebari(integ(H, W, Lista, Vocabular), [((_, _), Intrebare, _, _)|Rest]),
%    solutii_posibile(integ(H, W, Lista, Vocabular), [(Intrebare1, List1)|(Intrebare2, List2)|R]),
%    rezolvare_helper(integ(H, W, Lista, Vocabular), [(Intrebare1, List1)|(Intrebare2, List2)|R]).
