...
(define (queue-empty? q)
  (if (and (null? (queue-left q)) (null? (queue-right q))) true false))
...
(define (dequeue q)
  (if (null? (queue-left q))
      (struct-copy queue q [left (cdr (reverse (queue-right q)))] [right null] [size-l (sub1 (queue-size-r q))] [size-r 0])
