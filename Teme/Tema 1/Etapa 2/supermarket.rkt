...
(define (empty-counter index)
  (define C1 (make-counter index 0 0 '()))
  C1)
...
(define (add-to-counter name n-items)
  (λ (C)
    (struct-copy counter C
                 ...
                 [et (+ (if (= (length (counter-queue C)) 0) n-items 0) (counter-et C))]
...
(define (remove-first-from-counter C)
  (struct-copy counter C
               [tt (if (or (= (length (counter-queue C)) 0) (null? (cdr (counter-queue C)))) 0 (- (counter-tt C) (counter-et C)))]
               [et (if (or (= (length (counter-queue C)) 0) (null? (cdr (counter-queue C)))) 0 (cdar (cdr (counter-queue C))))]
               [queue (if (or (= (length (counter-queue C)) 0) (null? (cdr (counter-queue C)))) '() (cdr (counter-queue C)))]))
...
(define (serve requests fast-counters slow-counters)
  ...
  (if (null? requests)
      ...
      (match (car requests)
        ...
        [(list 'ensure average)
         (serve (cdr requests) fast-counters (ensure-helper fast-counters slow-counters average))]
        ...
        [(list 'remove-first)
         (if (null? (filter (λ (C) (not (null? (counter-queue C)))) (append fast-counters slow-counters)))
             (serve (cdr requests) fast-counters slow-counters)
             (serve (cdr requests)
                    (update remove-first-from-counter fast-counters (car (min-et (filter (λ (C) (not (null? (counter-queue C)))) (append fast-counters slow-counters)))))
                    (update remove-first-from-counter slow-counters (car (min-et (filter (λ (C) (not (null? (counter-queue C)))) (append fast-counters slow-counters)))))))])))            
