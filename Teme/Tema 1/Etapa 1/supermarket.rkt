...
(define (empty-counter index)
  (define C1 (make-counter index 0 '()))
  C1)
...
(define (min-tt counters)
 ...
 (define (min-tt-helper min counters)
   ; Am terminat de parcurs lista de counters, intoarcem perechea gasita
   (if (null? counters)
       (cons (counter-index min) (counter-tt min))
       ; Daca elementul actual este mai mic decat min, apelam functia cu noul min
       (if (< (counter-tt (car counters)) (counter-tt min))
           (min-tt-helper (car counters) (cdr counters))
           ; Altfel, daca sunt egale, verificam care index este mai mic
           (if (= (counter-tt (car counters)) (counter-tt min))
               ; Daca actualul element are indexul mai mic, apelam functia cu acesta in loc de min
               (if (< (counter-index (car counters)) (counter-index min))
                   (min-tt-helper (car counters) (cdr counters))
                   (min-tt-helper min (cdr counters)))
           ; Daca actualul min este cel mai mic, continuam parcurgerea cu acelasi min 
           (min-tt-helper min (cdr counters)))   
   )))
...
(define (add-to-counter C name n-items)
  (struct-copy counter C [tt (+ n-items (counter-tt C))] [queue (append (counter-queue C) (list (cons name n-items)))]))
...
(define (serve requests C1 C2 C3 C4)
  ...
  (define (add-helper-C1 index name n-items)
    (if(= index (counter-index C1))
       (if (<= n-items ITEMS)
           (serve (cdr requests) (add-to-counter C1 name n-items) C2 C3 C4)
           (add-helper-counters (car (min-tt (list C2 C3 C4))) name n-items))
    (add-helper-counters index name n-items)))

  (define (add-helper-counters index name n-items)
    (cond
      ((= index (counter-index C2)) (serve (cdr requests) C1 (add-to-counter C2 name n-items) C3 C4))
      ((= index (counter-index C3)) (serve (cdr requests) C1 C2 (add-to-counter C3 name n-items) C4))
      ((= index (counter-index C4)) (serve (cdr requests) C1 C2 C3 (add-to-counter C4 name n-items)))))

  (if (null? requests)
      (list C1 C2 C3 C4)
      (match (car requests)
        [(list 'delay index minutes) (delay-helper index minutes)]
        [(list name n-items)         (add-helper-C1 (car (min-tt (list C1 C2 C3 C4))) name n-items)])))
